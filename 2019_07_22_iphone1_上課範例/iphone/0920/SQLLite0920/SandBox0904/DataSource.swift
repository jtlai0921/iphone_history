...
class DataSource{
    ...
    func getCitysFromCountry(countryName:String) -> [City]!{
        guard sqlite3_open(documentsPath, &db) == SQLITE_OK else{
            print("sqlite3 db實體建立失敗");
            return nil;
        }
    ...
    func searchCity(key:String) -> [City]!{
        guard sqlite3_open(documentsPath, &db) == SQLITE_OK else{
            print("sqlite3 db實體建立失敗");
            return nil;
        }
        var statement:OpaquePointer!;
        let sqlStatement = "SELECT * FROM city WHERE name like ? OR continent like ? OR country like ?"
        ...
        sqlite3_bind_text(statement, 3, ("%\(key)%" as NSString).utf8String, -1,nil)
